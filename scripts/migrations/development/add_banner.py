...
def set_banner(banner_text: str = "Development", banner_background_color: str = ""):
    ...
    view_dic = {
        "name": "wetech.dev_banner",
        "type": "qweb",
        "mode": "extension",
        "active": True,
        "inherit_id": env.ref("web.layout").id,
        "arch_db": f"""
            <data name="Neutralize Banner" inherit_id="web.layout" active="False">
    ...
    if view := view_env.search([("name", "=", view_dic["name"])]):
        ...
        LOGGER.info("Creating Banner view: %s", view_dic["name"])
